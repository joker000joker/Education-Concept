...
export const PaidEbooksPage: React.FC = () => {
  const navigate = useNavigate();

  const PAID_EBOOKS_CATEGORIES = [
    { id: 'ssc', title: 'SSC E-BOOKS', icon: SscEbooksIcon, bg: 'bg-blue-100', color: 'text-blue-500' },
    { id: 'railways', title: 'RAILWAYS E-BOOKS', icon: RailwaysEbooksIcon, bg: 'bg-emerald-100', color: 'text-emerald-500' },
    { id: 'state-exams', title: 'STATE EXAMS E-BOOKS', icon: StateExamsEbooksIcon, bg: 'bg-purple-100', color: 'text-purple-500' }
  ];
  ...
  return (
    <div className="px-4 py-6 pb-24 min-h-screen bg-[#F4F8FF]">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-xl font-bold text-slate-900 tracking-tight">Paid E-Books</h1>
      </div>

      <div className="grid grid-cols-1 gap-4">
        {PAID_EBOOKS_CATEGORIES.map((cat) => (
          <Link
            key={cat.id}
            to={`/paid-ebooks/${cat.id}`}
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-400 mt-0.5">Explore premium study materials</p>
            </div>
          </Link>
        ))}
      </div>
