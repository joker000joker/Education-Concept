...
export const FreeEbooksPage: React.FC = () => {
  const navigate = useNavigate();

  const FREE_EBOOKS_CATEGORIES = [
  ...
  return (
    <div className="px-4 py-6 pb-24 min-h-screen bg-[#F4F8FF]">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-xl font-bold text-slate-900 tracking-tight">Free E-Books</h1>
      </div>

      <div className="grid grid-cols-1 gap-4">
        {FREE_EBOOKS_CATEGORIES.map((cat) => (
          <Link
            key={cat.id}
            to={`/free-ebooks/${cat.id}`}
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-400 mt-0.5">Explore free study materials</p>
            </div>
          </Link>
        ))}
      </div>
